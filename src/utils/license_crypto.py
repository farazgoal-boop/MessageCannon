"""
Item 36 of the multi-product generalization pass: offline, machine-bound
license verification using Ed25519 signatures — replacing the previous
`PAID_PASSKEY` scheme (a single hardcoded string, identical for every
buyer, compiled directly into the shipped EXE — anyone who ever recovered
that one string, e.g. by decompiling the exe, could activate every install
of this app for free, with no way to tell one buyer's activation from
another's). Directly mirrors JobMind Match's real, proven pattern (studied
from its own source before writing any of this, per this file's own
standing "study before building" discipline) — same request/activation-code
shape, same Ed25519 asymmetric scheme, same never-ship-the-private-key
boundary — adapted to this app's own naming.

Flow: the app derives a REQUEST CODE from a stable per-machine fingerprint
and shows it to the user, who sends it to the seller. The seller runs
scripts/generate_license.py (which holds the ONLY copy of the private key,
kept entirely outside this repo, never shipped) to sign that exact string,
producing an ACTIVATION CODE unique to that one machine. This module
verifies that signature using only the embedded PUBLIC key — a public key
reveals nothing that lets anyone forge a new signature or activate a
different machine with it.
...
# A real keypair was generated for this pass (private key kept outside this
# repo, at ~/.messagecannon-license-signing/private_key.pem, per this
# module's own docstring) so the whole flow could be proven end to end, not
# left as an inert placeholder.
PUBLIC_KEY_B64 = "U63KFL/2vZWVZCrsD9OANEgqT+wSAx7OklayADPQPw0="
...
def machine_fingerprint() -> str:
    ...
    if not payload:
        payload = machine_name() or "messagecannon"
...
def verify_activation_code(request_code: str, activation_code: str) -> bool:
    ...
    try:
        ...
        # still just "not a valid code", never a crash.
        return False
