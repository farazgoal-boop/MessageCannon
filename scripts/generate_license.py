...
key is ever loaded -- it lives at ~/.messagecannon-license-signing/
private_key.pem (outside the repo entirely, never committed, never shipped
in the app). src/utils/license_crypto.py holds only the public key and can
verify signatures but never produce new ones.

Mirrors JobMind Match's own scripts/generate_license.py -- same shape,
adapted for MessageCannon Pro's own request/activation prefixes.
"""
...
from src.utils.license_crypto import (  # noqa: E402
    _normalize_code,
...
def default_key_path() -> Path:
    return Path.home() / ".messagecannon-license-signing" / "private_key.pem"


def default_ledger_path() -> Path:
    return Path.home() / ".messagecannon-license-signing" / "issued_licenses.log"
...
def init_keys(key_path: Path) -> int:
    ...
    print()
    print("Paste this into src/utils/license_crypto.py's PUBLIC_KEY_B64:")
    print()
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="MessageCannon Pro seller tool: sign a buyer's request code into an activation code."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--key-path", type=Path, default=None, help="Override the private key path (default: ~/.messagecannon-license-signing/private_key.pem)"
    )
    parser.add_argument(
        "--ledger-path", type=Path, default=None, help="Override the issued-licenses ledger path (default: ~/.messagecannon-license-signing/issued_licenses.log)"
    )
